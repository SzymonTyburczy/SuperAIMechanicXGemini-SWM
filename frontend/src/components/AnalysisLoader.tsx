      ...
      <div className="w-full max-w-sm px-6">
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <div key={i} className="flex items-center gap-3">
                ...
                >
                  {isDone ? (
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#4caf78" strokeWidth="3">
                      <polyline points="20 6 9 17 4 12" />
                    </svg>
                  ) : isActive ? (
                    <svg className="animate-spin" width="10" height="10" viewBox="0 0 24 24" fill="none">
                      <circle cx="12" cy="12" r="9" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeDasharray="15 42" strokeLinecap="round" />
                    </svg>
                  ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-0.5">
                    <span
                      ...
                      style={{
                        color: isDone ? "rgba(76,175,120,0.8)" : isActive ? "rgba(255,255,255,0.7)" : "rgba(255,255,255,0.2)",
                      }}
                    >
                      {step.label}
                    </span>
                    {isDone && <span className="text-[10px] text-[#4caf78]/60">✓</span>}
                    {isActive && (
                      <span className="text-[10px] text-white/30">{Math.round(stepProgress)}%</span>
                    )}
                  </div>
                  {isActive && (
        ...
        <div>
          <div className="flex justify-between text-[11px] text-white/25 mb-2">
            <span>Progress</span>
            <span className="font-medium text-white/40">{Math.round(progress)}%</span>
          </div>
